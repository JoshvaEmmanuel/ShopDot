...
public class Integrations extends BaseClase{
	...
	public void user_Login_to_the_Shopdot_application() {
		driver.findElement(By.xpath("//input[@placeholder='Email address']")).sendKeys("joshva14@yopmail.com");
...
//006
	@When("the user enters an existing Shopify store name in the connection field")
	public void the_user_enters_an_existing_Shopify_store_name_in_the_connection_field() {
	    driver.findElement(By.xpath("//input[@type='text']")).sendKeys(propertiyReader.getProperty("existingIntgiration"));
	}


	@When("displays a Error message if the store name exists")
	public void displays_a_Error_message_if_the_store_name_exists() {
		 String existingerror = driver.findElement(By.xpath("//div[contains(text(),'This shop already exists with another user. Please')]")).getText();
	     assertEquals(existingerror, "This shop already exists with another user. Please try with a different shop");
		   System.out.println("the error is :" + existingerror);
	}
